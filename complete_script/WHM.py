...
class Augmentation:
    ...
    def wavelet_hybrid_preserve(
        ...
        """
        Wavelet-Hybrid-Preserve: Masks low-energy coefficients and mixes mid-energy ones within channels.
        
        Args:
            x: Input tensor (batch_size, seq_len, channels)
            y: Target tensor (batch_size, pred_len, channels)
            mask_rate: Fraction of low-energy coefficients to mask
            mix_k: Number of mid-energy coefficients to mix
            wavelet: Wavelet type for DWT
            level: Number of decomposition levels
            dim: Dimension to apply DWT
            perturb_scale: Scale of perturbation for masked coefficients
        ...
        # Compute energy for coefficient selection
        cA_energy = cA_abs / (cA_abs.sum(dim=-1, keepdim=True) + 1e-8)
        cDs_energy = [
            cD_abs / (cD_abs.sum(dim=-1, keepdim=True) + 1e-8) for cD_abs in cDs_abs
        ]

        # Mask low-energy coefficients (bottom mask_rate)
        mask_cA = torch.zeros_like(cA, dtype=torch.bool)
        for b in range(batch_size):
            for c in range(channels):
                _, indices = cA_energy[b, c].sort()
                mask_cA[b, c, indices[:int(mask_rate * len(indices))]] = True

        masked_cDs = []
        for cD, cD_energy in zip(cDs, cDs_energy):
            mask_cD = torch.zeros_like(cD, dtype=torch.bool)
            for b in range(batch_size):
                for c in range(channels):
                    _, indices = cD_energy[b, c].sort()
                    mask_cD[b, c, indices[:int(mask_rate * len(indices))]] = True
            masked_cDs.append(mask_cD)

        # Select mid-energy coefficients for mixing (middle 20%)
        mix_mask_cA = torch.zeros_like(cA, dtype=torch.bool)
        for b in range(batch_size):
            for c in range(channels):
                _, indices = cA_energy[b, c].sort()
                start_idx = int(0.4 * len(indices))  # Skip bottom 40%
                end_idx = min(start_idx + mix_k, int(0.6 * len(indices)))  # Middle 20%
                mix_mask_cA[b, c, indices[start_idx:end_idx]] = True

        mix_masked_cDs = []
        for cD, cD_energy in zip(cDs, cDs_energy):
            mix_mask_cD = torch.zeros_like(cD, dtype=torch.bool)
            for b in range(batch_size):
                for c in range(channels):
                    _, indices = cD_energy[b, c].sort()
                    start_idx = int(0.4 * len(indices))
                    end_idx = min(start_idx + mix_k, int(0.6 * len(indices)))
                    mix_mask_cD[b, c, indices[start_idx:end_idx]] = True
            mix_masked_cDs.append(mix_mask_cD)

        # Temporal-aware mixing weights
        b_idx = torch.randperm(batch_size)
        # Use cA.shape[-1] for temporal weights to match coefficient length
        temporal_weights = torch.linspace(0.1, 1.0, steps=cA.shape[-1], device=cA.device).view(1, 1, -1)
        mix_weights_cA = torch.rand(cA.shape, device=cA.device, dtype=torch.float) * 0.3 * temporal_weights
        mix_weights_cA = mix_weights_cA * mix_mask_cA.float()

        cA_mixed = cA.clone()
        cA_mixed[mix_mask_cA] = (1 - mix_weights_cA[mix_mask_cA]) * cA[mix_mask_cA] + mix_weights_cA[mix_mask_cA] * cA[b_idx][mix_mask_cA]

        cDs_mixed = []
        for cD, mix_mask in zip(cDs, mix_masked_cDs):
            cD_mixed = cD.clone()
            temporal_weights_cD = torch.linspace(0.1, 1.0, steps=cD.shape[-1], device=cD.device).view(1, 1, -1)
            mix_weights = torch.rand(cD.shape, device=cD.device, dtype=torch.float) * 0.3 * temporal_weights_cD
            mix_weights = mix_weights * mix_mask.float()
            cD_mixed[mix_mask] = (1 - mix_weights[mix_mask]) * cD[mix_mask] + mix_weights[mix_mask] * cD[b_idx][mix_mask]
            cDs_mixed.append(cD_mixed)

        # Apply perturbation to masked (low-energy) coefficients
        cA_mixed[mask_cA] = 0.0  # Mask low-energy coefficients
        cA_mixed[mask_cA] += torch.randn_like(cA_mixed[mask_cA], dtype=torch.float64) * perturb_scale
        for i, (cD, mask) in enumerate(zip(cDs_mixed, masked_cDs)):
            cDs_mixed[i][mask] = 0.0
            cDs_mixed[i][mask] += torch.randn_like(cDs_mixed[i][mask], dtype=torch.float64) * perturb_scale
...
def train(
    ...
    path = "./checkpoints/Wavelet-Hybrid-Preserve"
    ...
    for epoch in range(epochs):
        ...
        for i, (batch_x, batch_y, aug_data) in enumerate(train_loader):
            ...
            xy = aug.wavelet_hybrid_preserve(
                batch_x,
                batch_y[:, -pred_len:, :],
                mask_rate=mask_rate,
                mix_k=mix_k,
...
dataset_configs = {
    "ETTh1": {
        ...
        "aug_types": ["Wavelet-Hybrid-Preserve"],
        "aug_params": {
            96: {
                "Wavelet-Hybrid-Preserve": {
                    "mask_rate": 0.3,
                    "mix_k": 5,
                    "wavelet": "sym4",
                    "level": 2,
...
if __name__ == "__main__":
    ...
    for dataset_name, config in dataset_configs.items():
        ...
        for pred_len in config["pred_lens"]:
            ...
            # Run experiment for Wavelet-Hybrid-Preserve
            aug_type = "Wavelet-Hybrid-Preserve"
        ...
        # Save results
        with open(f"results_{dataset_name}_wavelet_hybrid_preserve.txt", "w") as f:
            for pred_len in config["pred_lens"]:
                for aug_type in config["aug_types"]:
